...
namespace OnlineActivity.EFCore.Infra
{
    ...
    {
        protected OnlineActivityDbContext context;

        public RepositoryBase(OnlineActivityDbContext context)
        {
        ...
        {
            var entityToUpdate = context.Set<TEntity>().Find(key);

            if(entityToUpdate != null)
            {
                context.Entry<TEntity>(entityToUpdate).CurrentValues.SetValues(entity);
                context.SaveChanges();
            }
        ...
        {
            var entityToUpdate = await context.Set<TEntity>().FindAsync(key);
